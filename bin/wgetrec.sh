...
log_dir=${log_dir:-"${HOME}/data/log/${dset_path}"}
mkdir -p $log_dir
...
rejects=${rejects-'*.js,*.css,*.ico,*.txt,*.gif,*.jpg,*.jpeg,*.png,*.mp3,*.pdf,*.tgz,*.flv,*.avi,*.mpeg,*.iso'}
ignore_tags=${ignore_tags-'img,link,script'}

echo "Recursively (to level ${level}, with limit/wait '${limit_rate}' / '${wait}') fetching '$@', logging to ${log_file}"
echo "wget -r -l${level} --no-clobber --no-parent
    --no-verbose -a ${log_file} -E
    -erobots=off ${wait} $limit_rate
    --reject "$rejects" --ignore-tags=$ignore_tags
