...
groups.each{ group, files ->
	def orig_tv_f = 'TV Shows/{n}/{episode.special ? "Special" : "Season "+s}/{n} - {episode.special ? "S00E"+special.pad(2) : s00e00} - {t}'
	def tv_format = '/Media/TV/{n}/{n+" S"+s.pad(2)}/{n} - {sxe} - {t}'
	// EPISODE MODE
	if (group.tvs && !group.mov) {
		rename(file:files, format:tv_format, db:'TheTVDB')
	}
	
	def orig_movie_f = 'Movies/{n} ({y})/{n} ({y}){" CD$pi"}'
	def movie_format = '/Media/MoviesToWatch/{genres[0]}/{n} ({y}{", "+director}){" CD"+pi}'
	// MOVIE MODE
	if (group.mov && !group.tvs) {
		rename(file:files, format: movie_format, db:'TheMovieDB')
	}
	println ''
}